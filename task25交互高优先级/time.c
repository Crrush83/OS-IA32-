...
struct FIFO32 *timerfifo;//timer的全局通知器！！！
//重新设计一个缓友好好的
...
//有新的定时器插入时
//fifo域也要填
struct TIMER* new_timer(unsigned int timeout,unsigned int id,struct FIFO32 *bind_fifo){
...
//
    ...
    for(node = timerman->dumbhead.next;node!=&(timerman->dumbtail);){
        ...
        if(t->timeout == 0){//集中更新
            ...
            fifo32_put(t->fifo,t->id);//利用全局定时器通知缓冲区通知到时事件
            }
