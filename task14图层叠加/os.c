...
int main(void) {
 ...
 // extern char hankaku[4096];
  ...
  memman_init(memman);
  int memsize = memtest(0x120000, 0x211ffff)/(1024*1024);//2000000B = 0x20MB = 32MB//怎么输出得48
  sprintf(line,"%d MB",memsize); 
  debugPrint((unsigned char *)line);
    //向memman注册32MB
  memman_free(memman,0x120000,1024*1024*32);
  ...
  struct LAYER *bglayer =  layer_screen(320,200);
  int i;
  for(i = 0;i < 1024;i++){
        clockbg[i] = MOON;
  }
    ...
    // debugPrint(line);
       ...
       if(fifo8_status(keyfifo) + fifo8_status(mousefifo) > 0){
        ...
        //计算新坐标 隐去旧鼠标 描画新鼠标
          ...
          //layer_updown(layman,mouselayer,1);
